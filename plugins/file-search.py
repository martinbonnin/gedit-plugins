...
class SearchQuery:
    ...
    def loadDefaults (self, gclient):
        ...
        try:
            self.options = gclient.get_without_default(gconfBase+"/options").get_bool()
        except:
            self.options = False
    ...
    def storeDefaults (self, gclient):
        ...
        gclient.set_bool(gconfBase+"/options", self.options)
    ...
    def __init__ (self, query, resultHandler):
        ...
        if query.selectFileTypes:
            ...
            if fileTypeList:
                ...
                findCmd += [")"]
        if query.options:
            optionsSeq = query.optionsString.split();
            if optionsSeq:
                findCmd += ["("]
                for o in optionsSeq:
                    findCmd += [o];
...
class FileSearchWindowHelper:
    def __init__(self, plugin, window):
        ...
        self._lastOptions = RecentList(self.gclient, "recent_options")
    ...
    def on_cbOptions_toggled (self, checkbox):
        self.tree.get_widget('cboOptionsList').set_sensitive( checkbox.get_active() )
    ...
    def on_btnCurrent_clicked (self, button):
    ...
    def openSearchDialog (self, searchText = None):
        ...
        if searchText == "" and self._lastSearch != None:
            searchText = self._lastSearch;
        ...
        cboLastOptions = self.tree.get_widget('cboOptionsList');
        cboLastOptions.set_model(self._lastOptions.store)
        cboLastOptions.set_text_column(0)

        if not(self._lastOptions.isEmpty()):
            optionsString = self._lastOptions.topEntry()
            self.tree.get_widget('cboOptionsEntry').set_text(optionsString)
        else:
            self.tree.get_widget('cboOptionsEntry').set_text("")
        ...
        self.tree.get_widget('cbOptions').set_active(query.options)
        self.tree.get_widget('cboOptionsList').set_sensitive(query.options)
        ...
        while not(inputValid):
            ...
            optionsString = self.tree.get_widget('cboOptionsEntry').get_text()
        ...
        query.options = self.tree.get_widget('cbOptions').get_active()
        query.optionsString = optionsString;
        ...
        self._lastOptions.add(optionsString)
        query.storeDefaults(self.gclient)
        self._lastDir = searchDir
        self._lastSearch = searchText
